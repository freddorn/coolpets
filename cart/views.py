        ...
        if request.method == 'POST':
            ...
            if request.headers['Content-Type'] == 'application/json':
                ...
                if post_request.get('idChangedInput'):
                    response = process_changed_input_request(
                        request, post_request, cart)

                # if user deleted item from cart
                if post_request.get('orderItemId'):
                    response = process_delete_request(
                        request, post_request, cart)
                ...
                order = Order.objects.filter(
                    customer=request.user, paid=False).first()
    ...
    if not request.session.get('cart'):
        ...
        if request.method == 'POST':
            ...
            if order_form.is_valid():
                ...
                order.country = ShippingDestination.objects.get(
